"""
train_gemma.py — Gemma 3 270M (text-only decoder checkpoint)

Gemma3TextForSequenceClassification was only recently added to transformers
(the 270M/1B text-only checkpoints did NOT have sequence-classification
support for most of 2025 — only the >=4B multimodal variants did, via a
separate Gemma3ForSequenceClassification class). Your transformers 5.14.x
includes it. If AutoModelForSequenceClassification.from_pretrained() raises
a "no auto mapping found" error, your transformers install is older than
you think for this specific class — check with:
    python -c "from transformers import Gemma3TextForSequenceClassification"

NOTE: google/gemma-3-270m is a GATED model on Hugging Face. Before running
this script you must:
    1. Accept the license at https://huggingface.co/google/gemma-3-270m
    2. Run `huggingface-cli login` in PowerShell and paste an access token
       (or set the HF_TOKEN environment variable)

Run:  python train_gemma.py
"""
...
MODEL_ID = "google/gemma-3-270m"     # text-only checkpoint, no vision tower
USE_4BIT = True
MAX_LENGTH = 512
NUM_LABELS = 2
